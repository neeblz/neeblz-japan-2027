...
def reference() -> str:
    ...
    **Закрытыми три свёртки стоят друг под другом.** Это её правка 24 августа,
    ...
    времени. Раскладку держит CSS (`.ref-rows`).
    """
    ref = load_reference()
...
/* Три оставшиеся — друг под другом.
   ────────────────────────────────────────────────────────────────────
   ...
   линий между ними, ни обводок — три белые плоскости на переливе, разделённые
   воздухом. Порядок прежний, по времени: до вылета → деньги → такс-фри.
...
(function(){
  ...
  function paintCheck(){
    ...
    /* Итог, а не жильё. Раньше здесь стояло `data.housing.jpy`, и это было
       верно ровно до того дня, когда в чек вошли её записи: строка, вся работа
       которой — доказать сумму, стала называть другое число (¥290 675 при
       итоге ¥518 042) и доказывать не то. Молча, потому что жильё никуда не
       делось и цифра выглядела правдоподобной. */
    words.push("сложено из показанного: " + yen(sums.jpy) + " — жильё и твои записи");

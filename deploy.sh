...
local_door() {
  ...
  # Порядок дней — вторая ручка за той же дверью, и «та же дверь» это
  # утверждение, а не устройство: `_routes.json` можно однажды сузить, и
  # закрытой останется только та ручка, которую проверяют.
  code=$(curl -s -o /dev/null -w '%{http_code}' "http://127.0.0.1:$port/api/days")
  [ "$code" = "401" ] && ok=$((ok + 1)) \
    || echo "  ✗ ручка дней без печенья ответила $code вместо 401" >&2

  local plan
  plan=$(mktemp)
  curl -s -b "$jar" -o "$plan" "http://127.0.0.1:$port/api/days" || true
  if grep -q '"order"' "$plan"; then
    ok=$((ok + 1))
  else
    echo "  ✗ с паролем ручка дней не отдала порядок: $(head -c 120 "$plan")" >&2
  fi
  rm -f "$plan"
...
echo "  обе ручки — записи и порядок дней — закрыты без печенья и отвечают с ним"

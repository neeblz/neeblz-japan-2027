...
**1763 с 24 августа, поздний вечер: пятый блок справки — приложения.** Её
просьба: «добавь мне в полезное приложения, которые надо скачать для айфона».
Блок из десяти пунктов стоит странице **55 точек** (1708 → 1763) — то есть
ровно одну закрытую строку стопки: 44 точки свёртки и 10 зазора. Больше не
стоит ничего: все десять пунктов и шесть ссылок лежат под стрелкой, и высоты
там ноль, пока она не нажмёт.

Порог поднят с 1710 до 1765 по последнему измерению, как и всегда.

**Её потолок при этом не пробит, и это надо назвать числом, а не ощущением.**
«2 экрана это ок» — это 1800 при экране 900; 1763 — 0.98 от него. Запаса
осталось 37 точек, то есть **следующий блок справки в потолок уже не влезет**:
шестая свёртка стоит те же 54. Разговор про это — к ней, а не к сетке.
Подгонять вёрстку под порог значило бы прятать её же правку: блок она
попросила сама, и он занимает столько, сколько занимает.

Торговаться, если понадобится, по-прежнему есть чем и это по-прежнему её
решение: четыре свёртки над справкой держат 174 точки ради одной строки текста
каждая.
...
with sync_playwright() as pw:
    ...
    # Оставшиеся блоки стоят друг под другом — её правка 24 августа, вечером:
    # «до вылета, деньги и таксфри не нравится как разворачиваются. пусть
    # стоят друг под другом». До этого они стояли в ряд карточками, и здесь
    # проверялось ровно обратное: три верхних края на одной линии.
    #
    # Сколько их — считается по данным, а не прибито числом. Прибитая тройка
    # была бы тем же самым, из-за чего пятый блок и пропал: список из четырёх
    # в сборке молча решал, чему быть на экране. Здесь она бы вдобавок
    # промолчала — стопка из четырёх свёрток при ожидании трёх проверяется как
    # «первые три стоят столбиком», и новая осталась бы вне проверки.
    #
    ...
    folded = [b for b in data["blocks"] if b["id"] != "visa"]
    stacked = (len(box) == len(folded)
               and all(box[i + 1]["top"] >= box[i]["bottom"] for i in range(len(box) - 1))
               and len({b["left"] for b in box}) == 1)
    want(stacked, f"все {len(folded)} свёртки справки стоят друг под другом "
         f'({len(box)} на экране; верх: {[b["top"] for b in box]}, '
         f'низ: {[b["bottom"] for b in box]})')

...
with sync_playwright() as pw:
    ...
    # ── вторая запись, ценой в долларах
    #
    # Здесь был другой путь: «+ место» в карточке города, цена в долларах,
    # строка в карточке Асакусы. Кнопки не существует с 24 августа — Ни убрала
    # «хочу сходить» из карточек, а места вписывает в днях, — и шаг оставался
    # нажатием в пустоту, то есть падал ещё до всякой проверки.
    #
    # Доказывал он, кроме самой кнопки, одно: **введённый доллар остаётся её
    # числом**, а не пересчитывается туда-обратно ($12 → ¥1 907 → $13). Это и
    # осталось, её же руками и той же формой — цена в долларах есть у всех
    # видов записи. Что старая запись-место по-прежнему рисуется, сторожит
    # `wide.py` (прогон с поддельным хранилищем), а что её принимает ручка —
    # `round.sh`.
    page.click('.knob[data-add="booking"]')
    page.fill("#f-title", "Лавка сэндвичей")
    page.fill("#f-amount", "12")
    page.select_option('.pane select[name="currency"]', "usd")
    page.select_option('.pane select[name="state"]', "onsite")
    page.click(".pane .save")
    page.wait_for_timeout(900)

    where = page.locator("[data-mine-booking] .own", has_text="Лавка сэндвичей")
    want(where.count() == 1, f"записей «Лавка сэндвичей» {where.count()} (ждали одну)")
    said = where.first.inner_text()
    want("$12" in said, "доллар показан её числом, а не пересчитанным обратно")
    want("плачу на месте" in said, "и деньги стоят те, что она выбрала")
